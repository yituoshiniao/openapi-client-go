#版本号
#branch = test
envPath = cs_healthy/goods-center-logic/$(env)
dockerName = goods-center-logic-$(env)
currentBranch = $(shell git symbolic-ref HEAD | sed -e 's,.*/\(.*\),\1,')
find_str = "gitlab_intsig_net_cs-server2_services_goods-center-logic_"
replace_str = ""

swagger_file = "/goods-center-logic/api/swagger/swagger.json"
api_html_file = "/goods-center-logic/gen/html2/index.html"
api_tmp_html_file = "/goods-center-logic/gen/html2/api.html"
...
pull-image-v2:
	docker login -u '$(docker_uname)' -p $(docker_pwd)  registry.intsig.net \
	docker pull registry.intsig.net/$(envPath):$(branch)
	@echo "拉取成功版本:"$(branch)
...
build-image:
	env CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build  -o app cmd/server/main.go;\
    docker login -u $(docker_uname) -p $(docker_pwd)  registry.intsig.net; \
	docker build  -t  registry.intsig.net/$(envPath):$(branch)  -f Dockerfile . ;  \
	rm app
	@echo "构建成功版本:"$(branch)
...
#推送镜像image到hub
push-image:
	docker push registry.intsig.net/$(envPath):$(branch)
	@echo "推送成功版本:"$(branch)
...
#日志上报配置详见： https://doc.intsig.net/pages/viewpage.action?pageId=350650880
#运行
run-local:
	...
	docker run --name $(dockerName)   \
		...
		--log-opt syslog-address=udp://10.2.8.203:10058 \
	  	--restart always registry.intsig.net/$(envPath):$(branch)
...
#swag api 文档参数： https://blog.csdn.net/qq_41630102/article/details/128411210
#--tags="iap_apple" 生成那些tag的文档,多个用逗号[,]分割
doc-swag:
	#swag init -g cmd/server/main.go -o api/swagger
	swag fmt
	swag init   -md ./api/md --generatedTime=true \
		--parseDependency=true --parseInternal=true \
		--parseGoList=true --parseDepth=3 -g cmd/server/main.go \
		--generatedTime=true \
		-o api/swagger
	sleep 1
	yapi import || true
...
copy-doc:
	rm -rf ./gen/swagger
	cp -rf ./api/swagger ./gen #拷贝swagger到gen目录下
	@echo "success 成功"
...
#替换 gitlab_intsig_net_cs-server2_services_goods-center-logic_ 字符串为空字符串
replace-swag-json:
#	find_str = "gitlab_intsig_net_cs-server2_services_goods-center-logic_"
#	replace_str = ""
	$(shell (find ./api/swagger -iname "*.json") | xargs -n1 -IX bash -c 'sed s/$(find_str)/$(replace_str)/ X > X.tmp && mv X{.tmp,}')
	$(shell (find ./api/swagger -iname "*.go") | xargs -n1 -IX bash -c 'sed s/$(find_str)/$(replace_str)/ X > X.tmp && mv X{.tmp,}')
	$(shell (find ./api/swagger -iname "*.yaml") | xargs -n1 -IX bash -c 'sed s/$(find_str)/$(replace_str)/ X > X.tmp && mv X{.tmp,}')
	@echo "swagger 成功"


#支持ssh地址,当不支持ssh地址可以使用http
#git subsplit publish  --no-tags --heads=$(currentBranch) gen:git@gitlab.intsig.net:cs-server2/services/openapi-client.git
...
gen-subsplit:
	...
	rm -rf .subsplit
	git subsplit init .
	git subsplit publish  --no-tags --heads=$(currentBranch) gen:https://gitlab.intsig.net/cs-server2/services/openapi-client.git #不支持ssh,使用它http同步
	git subsplit publish  --no-tags --heads=$(currentBranch) gen/go:https://gitlab.intsig.net/cs-server2/openapi-client-go.git
	git subsplit publish  --no-tags --heads=$(currentBranch) gen/lua:https://gitlab.intsig.net/cs-server2/services/openapi-client-lua.git
	rm -rf .subsplit
	@echo "执行成功"
...
openapi-doc-server:
	rm -rf ./api/swagger/go
	npx @openapitools/openapi-generator-cli generate -i ./api/swagger/swagger.json \
		-g go-gin-server \
	 	-o ./gen/go-gin-server
		@echo "执行成功"
...
#生成html文档 #-c "./api/swagger/config.json"
out_path_html =  "/gen/html2"
openapi-doc-html:
	rm -rf .$(out_path_html)
	npx @openapitools/openapi-generator-cli generate \
		-i ./api/swagger/swagger.json \
...
#git 仓库host
git-host = "gitlab.intsig.net"
#git仓库host后的路径
git-user-id = "cs-server2/services"

##指定运行容器版本  -v ${PWD}:/goods-center-logic openapitools/openapi-generator-cli:v7.5.0 generate \
...
#go 生成 openapi客户端路径
out_go_path  = "/goods-center-logic/gen/go"
openapi-doc-docker-go:
	rm -rf ..$(out_go_path)
	docker run --rm \
		-v ${PWD}:/goods-center-logic openapitools/openapi-generator-cli generate \
		-i $(swagger_file) \
		-g go \
		--git-host $(git-host) \
		--git-user-id "cs-server2"  \
...
out_lua_path  = "/goods-center-logic/gen/lua"
...
out_java_path = "/goods-center-logic/gen/java"
