...
import (
	"context"
	"fmt"
	"github.com/yituoshiniao/gin-api-http/dao/query"
...
func (r *UserScoreRepo) AddOne(ctx context.Context, userScore *entity.UserScore) (int64, error) {
	...
	user, err := table.WithContext(ctx).Where(table.UserID.Eq("1000150")).Debug().First()
	xlog.S(ctx).Infow("gen", "user", user, "err", "err")
